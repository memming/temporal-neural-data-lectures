...
\begin{parts}
    ...
    \begin{tcolorbox}
	Fun fact: expectation is linear, specifically, $E[aX] = aE[X]$ for a constant $a$.
    \end{tcolorbox}
    \vspace{\stretch{1}}
    \part Evaluate $\E[x_t y_t]$
    \begin{tcolorbox}
	Fun fact: if $X$ and $Y$ are independent, $E[XY] = E[X]E[Y]$
    \end{tcolorbox}
    \vspace{\stretch{1}}
...
\begin{parts}
    ...
    \end{equation}
    \begin{tcolorbox}
	Fun fact: Expectation is linear. $E[X+Y] = E[X] + E[Y]$
    \end{tcolorbox}
    \vspace{\stretch{1}}
\newpage
    \part
...
\begin{tcolorbox}
    Fun fact: $\var(aX) = a^2 \var(X)$.
\end{tcolorbox}
\begin{tcolorbox}
    Fun fact: if $X$ and $Y$ are independent, $E[f(X)g(Y)] = E[f(X)]E[g(Y)]$, also, $\var(X+Y) = \var(X) + \var(Y)$.
\end{tcolorbox}
    \vspace{\stretch{2}}
    \part What is the autocovariance at lag 1, $\cov(X_{t+1}, X_t)$?
    \vspace{\stretch{2}}
\newpage
    ...
    \vspace{\stretch{3}}
    \part \textbf{MATLAB Homework Q2:} Simulate AR(1) models with $a=0.9$ and $a=-0.9$ and $\sigma=0.01$ for $T=10,000$ time steps. Compute the sample mean and sample variance and plot the sample autocorrelation function. Compare these with the theoretical values you derived above. Is the process consistent with being \textit{ergodic}, that is, do the time average agree with the expectation?
    %\textbf{(Instruction: do all 7 MATLAB homeworks questions and submit the published PDF via BlackBoard.)}
    \part \textbf{MATLAB Homework Q3:} Let $x=x_{t-1}$ and $y=x_t$ and observe that Eq.~\ref{eq:ar1} is just a line with no bias. Simulate an AR(1) model with $a=-0.7$ and $\sigma=0.1$ for $T=10,000$ time steps. Given the time series generated by this AR(1) model, estimate the AR coefficient $a$ using linear regression (least squares).
\end{parts}
...
\begin{parts}
    ...
    \begin{tcolorbox}
	Fun fact: correlation coefficient is
	\begin{equation}
...
\end{tcolorbox}
\begin{parts}
    \part What is the power spectral density of white gaussian noise?
    \vspace{\stretch{2}}
    \part \textbf{MATLAB Homework Q3:} Generate $T=10,000$ long white gaussian noise with variance $2$ (i.e., standard deviation $\sqrt{2}$).
...
\begin{tcolorbox}
    ...
    \\
    Fun fact: absolute square of the \texttt{fft} of the raw signal is \underline{not} a (statistically) consistent estimator for the power spectral density. (Don't ever use this method!)
\end{tcolorbox}
\newpage
...
\begin{align}
    ...
    \\
    &= \frac{\sigma^2}{1-2a\cos(2\pi\omega)+a^2}
\end{align}
\part For what range of $a$ values does the AR(1) act as a \textit{low pass filter}, that is, its power spectrum peaks in low frequencies?
\begin{center}
\includegraphics[width=\textwidth]{ar_psd}
\end{center}
    \vspace{\stretch{1}}
    \part \textbf{MATLAB Homework Q4:} Estimate and plot the power spectral density of time series sampled from the two AR(1) models of \textcolor{red}{Homework Q1}.
\end{parts}
...
\begin{parts}
	...
	\begin{bmatrix}
	    X_t\\
	    X_{t-1}
	\end{bmatrix}$. Rewrite Eq.~\ref{eq:ar2} in matrix-vector form,
	\begin{align}
...
\end{tcolorbox}
    \newpage
    \part \textbf{MTALAB Homework Q5:} In general, when the \textit{eigenvalues} of $\mathbf{A}$ are of magnitude strictly less than 1, the corresponding AR(p) model is stable. Use MATLAB function \texttt{eig} to obtain eigenvalues of a matrix.
    ...
    \vspace{\stretch{1}}
    \part \textbf{MATLAB Homework Q7:} A better way of estimating AR model is to use MATLAB functions in the signal processing toolbox: \texttt{aryule, arcov, arberg}. Use each method and compare the result with the true model parameters.
    \vspace{\stretch{1}}
\end{parts}
...
\begin{parts}
    ...
    \vspace{\stretch{1}}
    \part What is the probability of firing an action potential in the current time bin if there were no spikes for the past $p$ time bins?
    \vspace{\stretch{2}}
    \part What is the log-likelihood of this model?
    \begin{tcolorbox}
	Fun fact: For a Poisson random variable $y \sim \text{Poisson}(\Lambda)$, its likelihood is,
	\begin{equation}
	    P(y\mid\Lambda) = 
...
\newpage
Do you have other task variables and external stimuli that may drive the neuron you are analyzing?
...
Note the logarithm is chosen as the ``link function''. This corresponds to the exponential inverse link function in~\eqref{eq:GLM2}.
