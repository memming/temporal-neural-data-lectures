...
Evaluate the \emph{expected} STA (discussed in~\citet{Dayan2001}), $\E[\sum_t y_{t} x_{t-\tau}]/\E[N]$ and sketch them for the following neuron models with indepedently random stimulus $x_t$ where $x_t = -1$ with probability $1/2$ and $x_t = 1$ with probability $1/2$.
\begin{parts}
    \part
    \begin{equation}
	y_t = 
